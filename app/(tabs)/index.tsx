...
export default function ScheduleScreen() {
  ...
  const [medicationsWithTaken, setMedicationsWithTaken] = useState<MedicationWithTaken[]>([]);
  const { loading, logMedication } = useMedications();
  ...
  const toggleMedication = async (medId: number, time: string) => {
    const medication = medicationsWithTaken.find(med => med.id === medId);
    if (!medication) return;

    const currentStatus = medication.taken[time];
    const newStatus = !currentStatus;

    setMedicationsWithTaken(prev =>
      prev.map(med =>
        med.id === medId ? { ...med, taken: { 
          ...med.taken, [time]: newStatus },
        } : med
      )
    );
    ...
    try {
      await logMedication({
        medicationId: medId,
        scheduledTime: time,
        actualTime: newStatus ? new Date().toTimeString().slice(0, 5) : undefined,
        date: selectedDate.toISOString().split('T')[0],
        status: newStatus ? 'taken' : 'missed',
      });
    } catch (error) {
      ...
      setMedicationsWithTaken(prev =>
        prev.map(med =>
          med.id === medId
            ? {
                ...med,
                taken: { ...med.taken, [time]: currentStatus },
              }
            : med
        )
      );
  ...
  const MedicationCard = ({ medication }: { medication: MedicationWithTaken }) => {
    return (
      <ScrollView style={styles.medicationHeader}>
    ...
    return (
      <ScrollView style={styles.content} showsVerticalScrollIndicator={true}>
        <View> { loading ? (
          ...
          ) : medicationsWithTaken.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No medications scheduled for today</Text>
            <Text style={styles.emptySubtext}>Add a medication to get started</Text>
          </View>
        ): (
            medicationsWithTaken.map((medication) => (
